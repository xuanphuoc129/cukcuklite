...
public class AddProductActivity extends BaseActivity implements IAddProductContract.IView, View.OnClickListener {

    private Products mProduct;

    private IAddProductContract.IPresenter mPresenter;

    EditText etProductName;

    TextView tvPriceProduct, tvUnitProduct, tvSaveButton;

    RelativeLayout rlColorContainer, rlThumbnailContainer;

    LinearLayout llPriceProduct, llUnitProduct , llStatus;
    ...
    */
    ...
    /**
     * Phương thức nhận sự kiện thêm mới sản phẩm thành công
     *
     * @created_by lxphuoc on 3/27/2019
     */
    @Override
    public void onAddNewSuccess() {
        showToast(getString(R.string.add_edit_message_success));
        LocalBroadcastManager.getInstance(this).sendBroadcast(new Intent(AppConstant.ADD_NEW_PRODUCT_BROADCAST));
        onBackPressed();
    }
    /**
     * Phương thức nhận sự kiện thêm mới sản phẩm thất bại
     *
     * @created_by lxphuoc on 3/27/2019
     */
    @Override
    public void onAddNewError() {
        showToast(getString(R.string.add_edit_message_error));
    }
        ...
        try {
            ...
            switch (id) {
                ...
                case R.id.tvSaveButton:
                    ...
                    mProduct.setProductStatus(1);
                    mPresenter.addProduct(mProduct);
                    break;
     ...
     * @created_by lxphuoc on 3/27/2019
     */
    private void updateProductThumbnail() {
        ivProductThumbnail.setImageBitmap(getBitmapFromAssets(mProduct.getProductThumbnail()));
    }

    /**
     * Phương thức hiển thị lỗi khi tên sản phẩm bỏ trống
     * @created_by lxphuoc on 3/27/2019
     */
    @Override
    public void showNameIsEmptyError() {
        showToast(getResources().getString(R.string.error_name_product_empty));
    }
